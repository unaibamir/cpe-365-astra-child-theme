...
<?php
	if($pmpro_invoice) {
		...
		$order_id = $pmpro_invoice->id;
		$pmpro_invoice->getUser();
		$pmpro_invoice->getMembershipLevel();
		
		$print_url = add_query_arg(array(
			"action"	=>	"cpe_print_invoice",
			"order"		=>	$order_id,
			"_wpnonce"	=>	wp_create_nonce( 'cpe_print_invoice-' . base64_encode($order_id) )
		...
		<a class="pmpro_a-print" target="_blank" href="<?php echo $print_url; ?>"><?php _e('Print', 'paid-memberships-pro' ); ?></a>
		<ul>
	...
	else
	{
		...
		{
			?>
			<table id="pmpro_invoices_table" class="pmpro_invoice table" width="100%" cellpadding="0" cellspacing="0" border="0">
			<thead>
				<tr>
