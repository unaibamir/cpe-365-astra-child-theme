...
<?php if ($course_has_plans && !$has_access): ?>
<div class="learndash-course-sidebar">
	<div class="sidebar-inner">
		<p><?php printf(esc_html_x('This %s is also available by purchasing the following subscriptions:', 'course', 'learndash'), esc_attr(LearnDash_Custom_Label::label_to_slug('course'))); ?></p>
		<ul>
			<?php
				foreach ($course_plans as $key => $plan) {
					$level = pmpro_getLevel($plan->membership_id);
					?>
			<li>
				<?php echo $level->name; ?> - <?php echo pmpro_formatPrice($level->initial_payment); ?>
			</li>
			<?php
				}
				?>
